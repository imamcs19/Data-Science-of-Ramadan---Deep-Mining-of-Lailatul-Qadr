...
def elq():
    ...
    sorted_data = sorted(filtered_data, key=lambda x: datetime.strptime(x, '%d-%m-%Y %H:%M'))

    sorted_data = np.array(sorted_data)
    # print(filtered_data)

    tgl_unik = sorted_data
    print(tgl_unik)
    byk_hari = len(tgl_unik)
    print("Banyak hari pengamatan = ", byk_hari)
    ...
    x_plot = (np.arange(byk_hari)+1).tolist()
    dates_list_plot = [tgl_unik_set[:-6] for tgl_unik_set in tgl_unik]
    # dates_list = [dt.datetime.strptime(date,"%d-%m-%Y") for date in dates]
    # dates_list_plot = [datetime.strptime(date,"%d-%m-%Y") for date in dates]
    # y1 = mean_data_suhu
    y1_as_mean_plot = mean_data_suhu

    # x2 = list(np.arange(byk_hari)+1)
    # y2 = var_data_suhu
    y2_as_var_plot = var_data_suhu
    ...
    # z = random.sample(range(100), 20)
    # size = random.sample(range(30), 20)
    # data = {'x': x, 'y': y, 'z': z, 'size': size}
    # return jsonify(data)

    # hasil_prediksi = {'Hasil Final idx_candidat_vote': idx_candidat_vote,\
    # 'Keterangan': "Tgl berikut sbg kandidat terjadinya LQ berdasar dari yg memiliki selisih terkecil antara abs( mean - suhu_tdk_dingin_tdk_panas) = "+str(tgl_unik[idx_candidat][:-6])+ "\natau tgl "+ str(tgl_ramadhan_nu[idx_candidat]) + " ramadhan nu dgn mean suhu = " + str(mean_data_suhu[idx_candidat]) + " dan var suhu = " + str(var_data_suhu[idx_candidat]),\
    # 'Data suhunya': str(raw_format_dataset_suhu[idx_candidat])}
...
# route that returns the HTML template
@app.route('/elq2')
def elq2():
    template = """
    <div class="container">
    ...
    <script>
      // create an empty trace for the chart
      const trace = {
        x: [],
        y: [],
        type: 'scatter',
      ...
      // create the chart and add the empty trace and layout
      ...
      setInterval(() => {
        fetch('/elq_data')
          ...
          .then(data => {
            // append the new data to the trace
            const update = {
              x: [[new Date()]],
              y: [[data.value]]
            };
...
# route that returns a JSON object with random data
@app.route('/elq_data')
def elq_data():
    import random
...
def elq_plot():
    ...
    x_dump = [i for i in range(1, 11)]
    y1_dump = [random.randint(1, 10) for i in range(10)]
    y2_dump = [random.randint(1, 10) for i in range(10)]

    # Generate random data for chart radar 1
    # categories = ["Category A", "Category B", "Category C", "Category D", "Category E"]
    # values1 = [random.randint(1, 5) for i in range(len(categories))]
    # values2 = [random.randint(1, 5) for i in range(len(categories))]

    city_names = ['Indonesia', 'Los Angeles', 'Chicago', 'Houston', 'Phoenix', 'Philadelphia', \
    ...
    values1 = [85, 75, 60, 50, 45, 70, 30, 65, 55, 80, 90, 35, 40, 50, 95, 70, 65, 75, 60, 80, 90]
    values2 = [75, 65, 50, 40, 35, 60, 20, 55, 45, 70, 80, 25, 30, 40, 85, 60, 55, 65, 50, 70, 80]

    # untuk radar dengan data dinamis dan komplek => cara ke-1
    list_kota = ['Jakarta','Los Angeles','Chicago','New York City','Toronto','São Paulo', \
             'Lagos', 'London', 'Johannesburg', 'Kairo', 'Paris', 'Zurich', 'Istanbul', 'Moskwa', 'Dubai', \
            'Mumbai','Hong Kong','Shanghai','Singapura','Tokyo','Sydney']
    ...
    data_kompleks = []
    for i in range(30):
        data_kompleks.append([random.randint(1, 10) for i in range(21)])

    # extend data, ditempatkan pada bagian akhir untuk membuat loop fill linenya terhubung
    # atau add in last element using first elemen value in each list multidimensional using insert
    # list_kota_v2 = list_kota.copy()
    # for sublist in list_kota_v2:
    #     last_element = sublist[0]
    #     sublist.insert(len(sublist), last_element)

    # Generate names dynamically
    names_kompleks = ['From ' + str(i) + ' days' for i in range(1, 31)]

    radar_data_kompleks = [{'type': 'scatterpolar',
                   'theta': list_kota,
                   'r': d,
                   'name': n,
                   'mode': 'lines',
                   ...
                   'fill': 'toself'
                  } for d, n in zip(data_kompleks, names_kompleks)]

    layout_kompleks = {'title': 'Plot Radar - Data Dummy 2 ', 'polar': {'radialaxis': {'visible': True, 'range': [0, 10]}},
              'showlegend': True,
              'legend': {'x': 0.05, 'y': 0.95},
             }

    # untuk radar komplek => cara ke-2
    # list_kota = ['Jakarta','Los Angeles','Chicago','New York City','Toronto','São Paulo', \
    #          'Lagos', 'London', 'Johannesburg', 'Kairo', 'Paris', 'Zurich', 'Istanbul', 'Moskwa', 'Dubai', \
    #         'Mumbai','Hong Kong','Shanghai','Singapura','Tokyo','Sydney']
    # byk_kota = len(list_kota)
    # cities = ['City ' + str(i) for i in range(1, byk_kota+1)]
    ...
    # response = requests.get('https://enterumum.pythonanywhere.com/elq')

    # # parse JSON response
    # data_from_api = response.json()

    # # extract x, y1, and y2 data
    # # 'x':x_plot,'y1':y1_as_mean_plot,'y2':y2_as_var_plot,'date':dates_list_plot
    # x = [d['x'] for d in data_from_api]
    # y1 = [d['y1'] for d in data_from_api]
    # y2 = [d['y2'] for d in data_from_api]


    # # Create the Plotly figure
    fig = {
        'data': [
            {'x': x_dump, 'y': y1_dump, 'type': 'scatter', 'name': 'Line 1'},
            {'x': x_dump, 'y': y2_dump, 'type': 'scatter', 'name': 'Line 2'}
        ],
        'layout': {
    ...
    # Render the template with the Plotly figure
    template = '''
    <!doctype html>
    <html>
    <head>
        <meta charset="utf-8">
        <title>Line Chart with Two Lines & Real-time Plot</title>
        <script src="https://cdn.plot.ly/plotly-latest.min.js"></script>
    ...
    <body>
        <div class="container">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div id="radar-kompleks-1"></div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div id="radar-kompleks-2"></div>
                </div>
            </div>
        </div>
        ...
        <script>
          ...
          // create a layout for the chart
          const layout = {
            title: 'Real-Time Plot (update tiap 1 detik)',
            xaxis: { title: 'Time' },
            yaxis: { title: 'Value' }
          };
        ...
        <script>
            // set up an interval to fetch new data every second, dan
            // Fetch the data from the server-side endpoint using Fetch API
            setInterval(() => {
            fetch('/elq')
                .then(response => response.json())
                .then(data => {
                    ...
                    const layout2 = {
                        ...
                        yaxis: {
                            title: 'Y = Mean and Var. of Temperature'
                        }
                    };
                    Plotly.newPlot('plot2', [trace1, trace2], layout2);
                })
                .catch(error => {
                    console.log(error);
                });
    ...
    <!-- untuk radar kompleks ke-1 -->
    <script>
        var radarData_kompleks = {{ radar_data_kompleks | tojson }};
        var layout_kompleks = {{ layout_kompleks | tojson }};
        Plotly.newPlot('radar-kompleks-1', radarData_kompleks, layout_kompleks);
    </script>

    <! -- untuk radar kompleks ke-2 (real data) -->
    <script>
        setInterval(() => {
            fetch('/elq')
              .then(response => response.json())
              .then(data => {
                ...
                for (let i = 1; i <= 30; i++) {
                  const values = cities.map(city => data[city.x][i]);
                  const trace = {
                    type: 'scatterpolar',
                    r: values,
                    theta: cities,
                ...
                const layout = {
                  title: "Real-Time Plot Radar LQ (update tiap 1 detik)",
                  polar: {
                    radialaxis: {
                      visible: true,
                      range: [0, 10]
                ...
                Plotly.newPlot('radar-kompleks-2', chartData, layout);
              })
              .catch(error => console.error(error));
          }, 1000);
    ...
    '''
    return render_template_string(template, fig=fig, city_names=city_names, values1=values1, values2=values2, radar_data_kompleks = radar_data_kompleks, layout_kompleks=layout_kompleks)
